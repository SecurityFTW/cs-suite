...
def main():
    ...
    if (args.audit_ip):
        ...
        subprocess.check_output(['cat ./reports/aws_audit/%s/%s/delta/cloud_formation ./reports/aws_audit/%s/%s/delta/ses  ./reports/aws_audit/%s/%s/delta/sns ./reports/aws_audit/%s/%s/delta/sns | ansi2html > ./reports/aws_audit/%s/%s/delta/notification.html'  %(account_name, timestmp,account_name, timestmp,account_name, timestmp,account_name, timestmp,account_name, timestmp)  ],shell=True)
