...
for  aws_region in ap-south-1 eu-west-2 eu-west-1 ap-northeast-2 ap-northeast-1 sa-east-1 ca-central-1 ap-southeast-1 ap-southeast-2 eu-central-1 us-east-1 us-east-2 us-west-1 us-west-2;do
  ...
  for db in $dbs; do
    ...
    if [ ! "$check" ]; then
       printf "default,$account,$aws_region,null,WARNING,Scored,null,REDSHIFT_AUDIT,${RED}Redshift instance $db does not have version upgrades enabled${NC}\n"
    else
       printf "default,$account,$aws_region,null,PASS,Scored,null,REDSHIFT_AUDIT,${GREEN}Redshift instance $db has version upgrades enabled${NC}\n"
    fi
    # Check if audit logging is enabled
    check=`aws redshift describe-logging-status --region $aws_region --cluster-identifier $db |grep true`
    if [ ! "$check" ]; then
      printf "default,$account,$aws_region,null,WARNING,Scored,null,REDSHIFT_AUDIT,${RED}Redshift instance $db does not have logging enabled${NC}\n"
    else
      printf "default,$account,$aws_region,null,PASS,Scored,null,REDSHIFT_AUDIT,${GREEN}Redshift instance $db has logging enabled${NC}\n"
    fi
    ...
    if [ ! "$check" ]; then
      printf "default,$account,$aws_region,null,WARNING,Scored,null,REDSHIFT_AUDIT,${RED}Redshift instance $db does not have encryption enabled${NC}\n"
    else
      printf "default,$account,$aws_region,null,PASS,Scored,null,REDSHIFT_AUDIT,${GREEN}Redshift instance $db has encryption enabled${NC}\n"
    fi
    ...
    if [ ! "$check" ]; then
      printf "default,$account,$aws_region,null,WARNING,Scored,null,REDSHIFT_AUDIT,${RED}Redshift instance $db is not using KMS keys${NC}\n"
    else
      printf "default,$account,$aws_region,null,PASS,Scored,null,REDSHIFT_AUDIT,${GREEN}Redshift instance $db is using KMS keys${NC}\n"
    fi
    ...
    if [ ! "$check" ]; then
      printf "default,$account,$aws_region,null,WARNING,Scored,null,REDSHIFT_AUDIT,${RED}Redshift instance $db may be using the EC2-Classic platform${NC}\n"
    else
      printf "default,$account,$aws_region,null,PASS,Scored,null,REDSHIFT_AUDIT,${GREEN}Redshift instance $db is using the EC2-VPC platform${NC}\n"
    fi
    ...
    for group in $groups; do
      ...
      if [ ! "$check" ]; then
        printf "default,$account,$aws_region,null,WARNING,Scored,null,REDSHIFT_AUDIT,${RED}Redshift instance $db parameter group $group is not using SSL${NC}\n"
      else
        printf "default,$account,$aws_region,null,PASS,Scored,null,REDSHIFT_AUDIT,${GREEN}Redshift instance $db parameter group $group is using SSL${NC}\n"
      fi
    done
    ...
    if [ "$check" ]; then
      printf "default,$account,$aws_region,null,WARNING,Scored,null,REDSHIFT_AUDIT,${RED}Redshift instance $db is publicly available${NC}\n"
    else
      printf "default,$account,$aws_region,null,PASS,Scored,null,REDSHIFT_AUDIT,${GREEN}Redshift instance $db is not publicly available${NC}\n"
    fi
  done
done
